  ...
  return `
    <article class="blog-card" data-category="${escapeHtml(blog.categorySlug)}" data-search="${escapeHtml(searchText)}">
      ...
      <div class="blog-card-body">
        ...
        <a class="blog-read-link" href="blog-detail.html?slug=${encodeURIComponent(blog.slug)}">Read guide →</a>
...
function renderPopularPosts() {
  ...
  popularList.innerHTML = popularBlogs
    .map(
      (blog) =>
        `<li><a href="blog-detail.html?slug=${encodeURIComponent(blog.slug)}">${escapeHtml(blog.title)}</a></li>`
    )
    .join("");
}
